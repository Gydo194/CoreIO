...
class UserController {
    ...
    public static function login() {
        ...
        if(isset($_REQUEST[LOGIN_USERNAME_PARAM_NAME])&& isset($_REQUEST[LOGIN_PASSWORD_PARAM_NAME])) {
            ...
            $u = MysqlUserDAO::getInstance()->getUserByUsernameAndPassword($username, $password);
        }
        if(isset($_REQUEST[LOGIN_TOKEN_PARAM_NAME])) {
            $token = self::getRequestParameter(LOGIN_TOKEN_PARAM_NAME);
            $u = MysqlUserDAO::getInstance()->getUserByToken($token);
        }
        //if($u) {
        self::$user = $u;
    ...
    public static function logout() {
        SessionController::setValue("user", "");
        //destroy user data
    }
